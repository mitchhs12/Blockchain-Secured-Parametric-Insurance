...
task("functions-deploy-payout", "Deploys the CheckPayout contract")
  ...
  .setAction(async (taskArgs) => {
    ...
    console.log(`Deploying CheckPayout contract to ${network.name}`)

    console.log("\n__Compiling Contracts__")
    await run("compile")

    const autoClientContractFactory = await ethers.getContractFactory("CheckPayout")
    ...
    console.log(`\nCheckPayout contract deployed to ${autoClientContract.address} on ${network.name}`)
  })
