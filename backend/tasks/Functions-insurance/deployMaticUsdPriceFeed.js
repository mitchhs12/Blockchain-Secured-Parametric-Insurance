...
task("functions-deploy-maticusd", "Deploys the MaticUsd contract")
  ...
  .setAction(async (taskArgs) => {
    ...
    console.log(`Deploying MaticUsdPriceFeed contract to ${network.name}`)

    console.log("\n__Compiling Contracts__")
    await run("compile")

    const pricefeedContractFactory = await ethers.getContractFactory("MaticUsdPriceFeed")
    ...
    console.log(`\tMaticUsdPriceFeed contract deployed to ${pricefeedContract.address} on ${network.name}`)
  })
