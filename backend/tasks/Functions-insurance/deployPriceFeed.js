...
task("functions-deploy-pricefeed", "Deploys the PriceConsumerV3 contract")
  ...
  .setAction(async (taskArgs) => {
    ...
    console.log(`Deploying PriceConsumerV3 contract to ${network.name}`)

    console.log("\n__Compiling Contracts__")
    await run("compile")

    const pricefeedContractFactory = await ethers.getContractFactory("PriceConsumerV3")
    ...
    console.log(`\tPriceConsumerV3 contract deployed to ${pricefeedContract.address} on ${network.name}`)
  })
