...
task("functions-set-payout", "Initiates a request from a Functions client contract")
  ...
  .addOptionalParam("requestgas", "Gas limit for calling the setSourceCode function", 1_500_000, types.int)
  .setAction(async (taskArgs, hre) => {
    ...
    const clientContractFactory = await ethers.getContractFactory("CheckPayout")
    ...
    const spinner = utils.spin({
      text: `Submitting transaction for CheckPayout contract ${contractAddr} on network ${network.name}`,
    })
    ...
    await new Promise(async (resolve, reject) => {
      ...
      const requestTx = await clientContract.setSourceCode(request.source)
